...
class BinaryLoss(nn.Module):
    ...
    def forward(self, probs, labels, sensitivity, specificity):
        ...
        if valid_mask.sum() == 0:
            return torch.tensor(0.0, device=probs.device), torch.tensor(0.0, device=probs.device)       # 如果没有有效样本，返回零损失

        print(f"probs: {probs}, labels: {labels}, valid_mask: {valid_mask}")        
        # 处理多个标注者的情况
        ...
        print(f"valid_probs: {valid_probs}, valid_labels: {valid_labels}")
        # 确保标签值在[0,1]范围内
        valid_labels = torch.clamp(valid_labels, 0, 1)
        
        # 使用Binary_Loss的方法计算损失, 将标签转换为one-hot编码
        ...
        print(f"p_sum: {p_sum},loss_val: {loss_val}, loss: {loss}")
        # 计算准确率
...
class PreferenceTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, return_output_and_metrics=False):
        ...
        if conclusion_dim in labels:
            ...
            # 确保dim_labels的第一维与p匹配
            if p.shape[0] != dim_labels.shape[0]:
                # 如果p的批次大小是dim_labels的两倍(因为A和B各一个)，则需要调整
                if p.shape[0] == 1 and dim_labels.shape[0] > 1:
                    p = p.expand(dim_labels.shape[0], -1)      # 如果p只有一个样本但dim_labels有多个，复制p
                elif dim_labels.shape[0] == 1 and p.shape[0] > 1:
                    p = p[0].unsqueeze(0)       # 如果dim_labels只有一个样本但p有多个，只使用第一个p
