...
class SaMerModel(AutoModelForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
        # 替换分类器为SaMerClassifier
        hidden_size = self.config.hidden_size
        self.classifier = SaMerClassifier(hidden_size, config.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        # 使用SaMerClassifier获取分数和方差
        logits, var = self.classifier(pooled_output)
        
        # 返回包含logits和var的字典
        return {"logits": logits, "var": var}
...
def main():
    ...
    if args.model_name_or_path:
        ...
        # 创建SaMerModel并复制基础模型的参数
        model = SaMerModel.from_config(base_model.config)
