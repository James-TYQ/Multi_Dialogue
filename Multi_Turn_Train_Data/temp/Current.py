...
class SaMerClassifier(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.prediction_layer = nn.Sequential(
            ...
            nn.Linear(1024, output_dim*2, bias=False)
        )

    def forward(self, x):
        return self.prediction_layer(x)

# 然后在LlamaForMDQRwardModel中使用两个独立的分类器
class LlamaForMDQRwardModel(AutoModelForSequenceClassification):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        # 创建自定义输出类，兼容PyTorch 2.2.1
        class CustomSequenceClassifierOutput(transformers.modeling_outputs.SequenceClassifierOutput):
...
class PreferenceTrainer(Trainer):
    def __init__(self, *args, label_field=None, **kwargs):
        ...
        # 将0-1范围的值转换为logit值，确保是标量而非空张量
        initial_sensitivity_logit = torch.logit(torch.tensor(self.args.initial_sensitivity, dtype=torch.float32))
        initial_specificity_logit = torch.logit(torch.tensor(self.args.initial_specificity, dtype=torch.float32))
        
        # 确保参数是标量，形状为[1]而不是[]
        self.sensitivity = nn.Parameter(initial_sensitivity_logit.reshape(1))
        self.specificity = nn.Parameter(initial_specificity_logit.reshape(1))
        
        # 不在这里创建优化器，让Trainer自己处理
        logger.info(f"初始敏感度: {self.args.initial_sensitivity}, 初始特异度: {self.args.initial_specificity}")
    
    # 重写create_optimizer方法，确保与DeepSpeed兼容
    def create_optimizer(self):
        """创建优化器，确保敏感度和特异度参数被正确处理"""
        if self.optimizer is None:
            # 将敏感度和特异度参数添加到模型中
            if not hasattr(self.model, 'sensitivity'):
                self.model.sensitivity = self.sensitivity
                self.model.specificity = self.specificity
            
            # 创建参数组，确保每个组都有非空参数
            decay_parameters = [p for n, p in self.model.named_parameters() 
                               if p.requires_grad and p.numel() > 0 and 
                               not any(nd in n for nd in ["bias", "LayerNorm.weight", "sensitivity", "specificity"])]
            
            no_decay_parameters = [p for n, p in self.model.named_parameters() 
                                  if p.requires_grad and p.numel() > 0 and 
                                  any(nd in n for nd in ["bias", "LayerNorm.weight"]) and
                                  not any(nd in n for nd in ["sensitivity", "specificity"])]
            
            ss_parameters = [self.model.sensitivity, self.model.specificity]
            
            # 检查参数组是否为空
            if not decay_parameters:
                logger.warning("权重衰减参数组为空，使用默认参数组")
                decay_parameters = [p for n, p in self.model.named_parameters() 
                                   if p.requires_grad and p.numel() > 0 and
                                   not any(nd in n for nd in ["sensitivity", "specificity"])]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, return_output_and_metrics=False):
        ...
        if hasattr(outputs, 'logits'):
            logits = outputs.logits  # 模型输出的得分
        else:
            # 兼容不同的输出格式
            logits = outputs[0] if isinstance(outputs, tuple) else outputs
        
        # 获取方差，确保它存在
        var = getattr(outputs, 'var', None)
        if var is None:
            print("警告: 输出中没有方差属性，使用默认值")
            var = torch.ones_like(logits) * 1e-6
        
        print("outputs类型:", type(outputs))
        print("logits:", logits.shape if isinstance(logits, torch.Tensor) else type(logits))
        print("var:", var.shape if isinstance(var, torch.Tensor) else type(var))
        ...
        if conclusion_dim in labels:
            ...
            # 使用BinaryLoss计算损失，使用模型中的敏感度和特异度参数
            sensitivity_value = torch.sigmoid(model.sensitivity[0]).item()  # 注意这里使用[0]
            specificity_value = torch.sigmoid(model.specificity[0]).item()  # 注意这里使用[0]
            
            loss, accuracy = self.binary_loss(p, dim_labels, 
            ...
            # 如果没有结论维度，返回零损失
            self.log({"loss": 0.0, "accuracy": 0.0})
...
def main():
    ...
    #     model = AutoModelForSequenceClassification.from_config(config)
        if hasattr(model, 'score') and hasattr(model.score, 'in_features'):
            in_features, out_features = model.score.in_features, model.score.out_features
            model.score = SaMerClassifier(input_dim=in_features, output_dim=out_features//2)
        else:
            # 对于没有预定义score属性的模型，使用hidden_size作为输入维度
            model.score = SaMerClassifier(input_dim=config.hidden_size, output_dim=config.num_labels//2)
